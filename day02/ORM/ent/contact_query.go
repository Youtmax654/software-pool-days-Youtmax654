...
func (cq *ContactQuery) QueryArtist() *ArtistQuery {
	query := (&ArtistClient{config: cq.config}).Query()
...
func (cq *ContactQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (cq *ContactQuery) Clone() *ContactQuery {
	...
	return &ContactQuery{
		...
		withArtist: cq.withArtist.Clone(),
...
func (cq *ContactQuery) WithArtist(opts ...func(*ArtistQuery)) *ContactQuery {
	query := (&ArtistClient{config: cq.config}).Query()
...
func (cq *ContactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Contact, error) {
	...
	if query := cq.withArtist; query != nil {
		if err := cq.loadArtist(ctx, query, nodes, nil,
			func(n *Contact, e *Artist) { n.Edges.Artist = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ContactQuery) loadArtist(ctx context.Context, query *ArtistQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *Artist)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Contact)
	for i := range nodes {
		if nodes[i].artist_contact == nil {
			continue
		}
		fk := *nodes[i].artist_contact
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "artist_contact" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *ContactQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(contact.Table, contact.Columns, sqlgraph.NewFieldSpec(contact.FieldID, field.TypeUUID))
	_spec.From = cq.sql
